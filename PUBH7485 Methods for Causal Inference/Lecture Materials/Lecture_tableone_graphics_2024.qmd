...
-   Many R packages which can create a (pretty) summary table
...
- As an example, we will use the well known iris dataset which contains data on 150 irises including their species type, petal length and width, and sepal length and width
...
-   Can be useful to assess overlap and similarity in the distribution of covariates; will introduce more formally in the context of matching, weighting
-   Use cobalt package in R

## Overlap and Similarity of Age

```{r, size = "tiny", fig.height=4}
library(cobalt)
bal.plot(as.formula(PHN.C1 ~ (PERSONS + VOTE96.1 + NEW + MAJORPTY + 
		AGE + WARD)), data = imai, "AGE", which = "unadjusted")
```
