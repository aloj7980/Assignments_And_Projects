---
title: "Propensity Score and Weighting"
author: "David M. Vock"
format: beamer
theme: "Boadilla"
colortheme: "whale"
...
-   Estimate the ATE using \begin{equation}
    \hat{\delta} = \sum_{j=1}^5 (\overline{Y}_{1j} - \overline{Y}_{0j}) \frac{n_j}{n}
    \end{equation} where $(\overline{Y}_{1j}, \overline{Y}_{0j})$ are the sample average response among subjects in the jth quintile receiving treatments 1 and 0, respectively, and $n_j$ is the number of individuals in the jth quintile
...
3)  Estimate ATE using \begin{equation}
    \hat{\delta} = \sum_{j=1}^5 (\overline{Y}_{1j} - \overline{Y}_{0j}) \frac{n_j}{n}
    \end{equation}
